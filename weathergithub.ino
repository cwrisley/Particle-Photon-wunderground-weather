...
/******************************************************************************
 12/7/2015 : Veriosn 1.1 - Updated version #
             Version 1.1 - Updated liine 508 (No longer return -1 if no wind collected to comply with wunderground, as that is not a suitable variable)
...
Spark.publish("Version", "Weathergithub 1.1");
delay(2000);
Spark.publish("temperature", String(tempf) + " °F");
delay(2000);
Spark.publish("humidity", String(humidity) + "%");
delay(2000);
Spark.publish("Wind Direction", String(winddir) + ".");
delay(2000);
Spark.publish("Wind Speed", String(windspeedmph) + "mph");
delay(2000);
...
int get_wind_direction()
{
  ...
  if(adc > 2270 && adc < 2290) return (1);//North
